...
results <- rbindlist(lapply(c(10, 100, 500), function(n){
  rbindlist(lapply(c(700, 800, 900, 1000, 1100), function(trueVal){
    x_pois <- rpois(n, trueVal)
    x_norm <- rnorm(n, trueVal, 100)
    ...
    par_pois <- ppois(310, MLE_pois)
    par_norm <- pnorm(310, MLE_norm)
    
    #calculate analytic variance
    v_pois <- par_pois*(1-par_pois)
    v_norm <- par_norm*(1-par_norm)
...
```{r}
# Get results from simulation
bias_sim = results[, .(mean_bias = mean(prob - trueValue),
                           lower_bias = quantile(estimate - trueValue, 0.025),
                           upper_bias = quantile(estimate - trueValue, 0.975)),
by = c("n", "trueValue", "estimator")]
